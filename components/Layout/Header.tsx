...
import { GameTime, Player, AppSettings } from '../../types';
...
export const Header: React.FC<HeaderProps> = (props) => {
  const { 
    player, gameTime, currentLocation, isSaving, modals, setModals, handleBack, handleExit, view, onManualSave, onRetry, isLoading, settings, onUpdateSettings, markAsViewed 
  } = props;
  ...
  const hasNewHarem = player.relationships.some(r => r.type === 'harem' && r.viewed === false);
  const hasNewFamily = player.relationships.some(r => r.type === 'family' && r.viewed === false);
  const hasNewSocial = player.relationships.some(r => r.type === 'social' && r.viewed === false);
  ...
  const navButtons = [
    ...
    {id: 'harem', label: 'Harem', color: 'pink', key: 'F2', borderColor: 'border-pink-500/30', activeColor: 'bg-pink-500', textColor: 'text-pink-400', hasDot: hasNewHarem},
    {id: 'family', label: 'Family', color: 'orange', key: 'F3', borderColor: 'border-orange-500/30', activeColor: 'bg-orange-500', textColor: 'text-orange-400', hasDot: hasNewFamily},
    {id: 'social', label: 'Social', color: 'cyan', key: 'F4', borderColor: 'border-cyan-500/30', activeColor: 'bg-cyan-400', textColor: 'text-cyan-400', hasDot: hasNewSocial},
  ...
  const handleNavClick = (id: string) => {
    ...
    if (isOpening && markAsViewed) {
      if (id === 'harem' || id === 'family' || id === 'social') {
        player.relationships
          .filter(r => r.type === id && r.viewed === false)
          .forEach(r => markAsViewed(r.id, 'npc'));
      }
    }
  };

  return (
    <header className={`Header border-b border-white/5 bg-black/60 backdrop-blur-2xl shrink-0 z-50 transition-all duration-300 ${isCollapsed ? 'h-10' : 'h-auto min-h-20'}`}>
      <div className={`w-full px-4 md:px-8 flex items-center justify-between transition-all duration-300 ${isCollapsed ? 'h-10' : 'h-auto py-4 md:h-20'}`}>
        <div className={`flex items-center gap-3 md:gap-4 transition-opacity duration-500 ${isPlaying || isSubSelect ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          ...
          <div className="flex flex-col min-w-0">
            <h1 className={`font-black tracking-[0.2em] uppercase text-white leading-none transition-all ${isCollapsed ? 'text-[9px] mb-0' : 'text-xs md:text-sm mb-1'}`}>{player.name}</h1>
            {!isCollapsed && (
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="flex items-center gap-2">
            {isPlaying && !isCollapsed && !isMobile && (
              <button 
                onClick={onRetry} 
                disabled={isLoading}
        ...
        <div className="absolute top-full left-0 w-full bg-black/95 backdrop-blur-3xl border-b border-white/10 p-4 grid grid-cols-2 gap-2 animate-in slide-in-from-top-4 duration-300 z-[60]">
          ...
          {onRetry && (
            <button 
              ...
              className="px-3 py-4 bg-amber-500/10 border border-amber-500/40 text-amber-500 rounded-xl mono text-[10px] font-black uppercase"
            >
              🔄 Tải Lại
            </button>
          )}
