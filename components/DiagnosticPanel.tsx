...
export const DiagnosticPanel: React.FC<DiagnosticPanelProps> = ({ logs, isMobile, settings, player }) => {
    ...
    return logs.filter(log => log.type === 'error').map(log => {
      ...
      if (content.includes('api key') || content.includes('invalid')) {
        ...
        result = {
          ...
          solution: 'Nhấn nút "Tải Lại" (Refresh) trên Header để đồng bộ lại thực tại. Nếu lặp lại, hãy thử "Retry" hành động.',
          severity: 'low'
        };
  ...
  const MinimizedButton = (
    <button 
      onClick={handleOpen}
      className={`fixed z-[600] flex items-center justify-center transition-all duration-300 group ${
        isMobile ? 'bottom-20 right-4 w-8 h-8' : 'bottom-24 right-4 w-10 h-10'
      } bg-neutral-900/80 border border-white/10 rounded-full backdrop-blur-md shadow-2xl hover:scale-110 active:scale-95`}
    >
      <ShieldAlert className={`w-4 h-4 ${hasNewError ? 'text-rose-500 animate-pulse' : 'text-neutral-500 group-hover:text-white'}`} />
      {hasNewError && (
        <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-neutral-900 animate-bounce shadow-[0_0_10px_#f43f5e]"></div>
      )}
      <div className="absolute -bottom-8 right-0 bg-black/80 px-2 py-1 rounded text-[8px] font-black text-white uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
        Diagnostics
      </div>
    </button>
  );

  if (!isExpanded) return MinimizedButton;
          ...
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 opacity-30">
              <div className="flex items-center gap-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.5em]">MATRIX_ENGINE_DIAGNOSTICS_V4.0</span>
              </div>
