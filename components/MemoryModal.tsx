...
type SortBy = 'turn' | 'importance' | 'recent' | 'pinned';
type FilterType = 'all' | 'fact' | 'event' | 'relationship' | 'preference' | 'pinned';
type ScoreFilter = 'all' | 'high' | 'medium' | 'low';

export const MemoryModal: React.FC<Props> = ({ isOpen, onClose, settings }) => {
  ...
  const filteredAndSortedMemories = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'turn') {
        return b.metadata.turn - a.metadata.turn;
  ...
  if (settings.mobileMode) {
    return <MobileMemoryModal onClose={onClose} />;
  }
        ...
        {selectedMemory ? (
                ...
                <div className="flex flex-wrap gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 text-2xl font-black text-white italic">
                      ...
                      Lượt {selectedMemory.metadata.turn}
                    </div>
                  </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-12">
            <div className="relative mb-12">
              ...
              <Brain className="w-32 h-32 text-indigo-500/20 relative z-10 animate-pulse" />
            </div>
            ...
            <p className="max-w-md text-neutral-700 mono text-xs font-bold uppercase tracking-widest leading-relaxed">
              Chọn một mảnh ký ức từ danh sách bên trái để tái hiện lại sự kiện trong ma trận lượng tử.
            </p>
          </div>
        )}
