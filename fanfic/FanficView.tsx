...
import { ArrowLeft, Search, User, Users, BookOpen, Send, Sparkles, Plus, Trash2, LayoutGrid, PenTool, ChevronDown, ChevronUp, X, Home } from 'lucide-react';
import { FANFIC_WORKS } from './data';
import { FanficWork, FanficCharacter, FanficCountry } from './types';
import { AppSettings } from '../types';

type FanficTab = 'preset' | 'custom';
...
export const FanficView: React.FC<Props> = ({ onBack, onExit, onStartGame, settings }) => {
  ...
  const [newMcData, setNewMcData] = useState<FanficCharacter>({ name: '', role: '', description: '' });
  ...
  const [customMc, setCustomMc] = useState<FanficCharacter>({ name: '', role: '', description: '' });
  const [customNpcs, setCustomNpcs] = useState<FanficCharacter[]>([]);

  const filteredWorks = FANFIC_WORKS.filter(w => {
  ...
  const countryCounts = {
    'Tất cả': FANFIC_WORKS.length,
    'Trung': FANFIC_WORKS.filter(w => w.country === 'Trung').length,
    'Nhật': FANFIC_WORKS.filter(w => w.country === 'Nhật').length,
    'Hàn': FANFIC_WORKS.filter(w => w.country === 'Hàn').length,
    'Việt Nam': FANFIC_WORKS.filter(w => w.country === 'Việt Nam').length,
    'Khác': FANFIC_WORKS.filter(w => !w.country || !['Trung', 'Nhật', 'Hàn', 'Việt Nam'].includes(w.country)).length,
  };
  ...
  const addCustomNpc = () => {
    setCustomNpcs([...customNpcs, { name: '', role: '', description: '' }]);
  };
                ...
                <div className="space-y-3 md:space-y-6">
                  ...
                  <div className="space-y-2 md:space-y-4">
                    ...
                    <div>
                      ...
                      <textarea 
                        placeholder="Mô tả ngắn về bối cảnh, hệ thống sức mạnh..."
