...
export const PlayingView: React.FC<Props> = (props) => {
  const { player, genre, logs, isLoading, handleCommand, onOpenAiHint, openNpcProfile, settings } = props;
  ...
  const isFamily = (npc: Relationship) => {
    if (npc.type === 'family') return true;
    if (npc.type === 'harem' || npc.type === 'social') return false; // Ưu tiên lựa chọn thủ công
  ...
  const sortedNpcs = useMemo(() => {
    const filtered = player.relationships.filter(n => {
      ...
      const isHarem = n.type === 'harem' || (n.affinity || 0) >= 600;
    ...
    return [...filtered].sort((a, b) => {
      ...
      // 2. Ưu tiên Harem (Thiện cảm >= 600 hoặc type harem)
      const aHarem = a.type === 'harem' || (a.affinity || 0) >= 600;
      const bHarem = b.type === 'harem' || (b.affinity || 0) >= 600;
  ...
  return (
    <div className={`flex-grow overflow-hidden p-1 relative ${isMobile ? 'flex flex-col' : 'grid grid-cols-1 lg:grid-cols-12 gap-1'}`}>
      ...
      <DiagnosticPanel logs={logs} isMobile={isMobile} settings={settings} player={player} />
        ...
        <div className={`bg-[var(--bg)]/90 border border-white/5 rounded-sm p-2 flex flex-col h-full shadow-[0_0_40px_rgba(0,0,0,0.5)] backdrop-blur-3xl relative overflow-hidden group ${isSidebarCollapsed && !isMobile ? 'items-center' : ''}`}>
          ...
          {!isSidebarCollapsed ? (
            ...
            <div className="flex flex-col items-center gap-4 py-8 relative z-10">
               ...
               <span className="text-[8px] mono text-neutral-600 uppercase font-black tracking-widest [writing-mode:vertical-rl] rotate-180">Verified_Souls // {sortedNpcs.length}</span>
            </div>
          )}
          ...
          <div className="flex-grow overflow-y-auto custom-scrollbar pr-1 space-y-2 relative z-10">
            {sortedNpcs.length > 0 ? (
              sortedNpcs.map((npc, idx) => {
                ...
                const isFam = npc.type === 'family' || (!npc.type && isFamily(npc));
                        ...
                        <div className="flex-grow min-w-0 flex flex-col gap-0.5">
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-1.5 truncate">
                              ...
                              {isHarem ? (
                                <span className="text-[7px] px-1 bg-pink-500 text-black font-black rounded-sm shrink-0">HAREM</span>
                              ) : isFam ? (
                                <span className="text-[7px] px-1 bg-orange-500 text-black font-black rounded-sm shrink-0">FAMILY</span>
                              ) : null}
